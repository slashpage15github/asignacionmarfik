...
<head>
	<meta charset="UTF-8">
	<title>Modificación de Horario</title>
</head>
...
<script type="text/javascript">
	$( function() {
			...
			$('#f_hi').wickedpicker(options);
			$('#f_hf').wickedpicker(options2);
		});
</script>
...
<body>
	...
	<div class="container">
		...
		<div class="row">
			<div class="col-lg-12 text-center">
				<h1 class="sinMargen">Ajuste individual Ordinarios por alumno </h1>
		...
		<?php
			...
			$_matricula=""; $_alumno=""; $_clave=""; $_grupo="";$_expediente=""; $_maestro=""; $_materia=""; $_dia_ordi=""; 
			$_fecha_ordi=""; $_horai_ordi=""; $_horaf_ordi=""; $_aula_ordi=""; $_dia_extra=""; $_fecha_extra=""; 
			$_horai_extra=""; $_horaf_extra=""; $_aula_extra=""; $alumno="";
		...
		<div id="container">
			<form method="POST" action="log_forma_alumnos_abc.php" name="forma_ingreso" onsubmit="return Validate_usuarios_abc();">
            	...
            	<div class='col-md-12'>
                	<div class="col-md-2 form-group">
                    	...
                    	<input style='font-weight:bold;text-align:center;font-size:20px;' type="text" id="f_mat" name="f_mat" class="form-control" value="<?=$_matricula;?>"  readonly>
                	</div>
                	<div class="col-md-4 form-group">
                    	<label>Alumno</label>
                    	<input type="text" id="f_nombre" name="f_nombre" class="form-control" value="<?=utf8_encode($_alumno);?>"  readonly>
                	</div>
                	<div class="col-md-2 form-group">
                    	<label>Clave</label>
				...
				<div class='col-md-12'>
                	...
                	<div class="col-md-4 form-group">
                    	<label>Materia</label>
                    	<input type="text" id="f_curp" name="f_curp" class="form-control" value="<?=utf8_encode($_materia);?>"  readonly>
                	</div>
                	<div class="col-md-2 form-group">
                		<label>Fecha</label>
                    	<input type="text" id="f_fecha" name="f_fecha" class="form-control" value="<?=$_fecha_ordi;?>">
                	</div>
                	<div class="col-md-2 form-group">
                    	<label>Día</label>
                    	<input type="text" id="f_dia" name="f_dia" class="form-control" value="<?=utf8_encode($_dia_ordi);?>">
                    </div>
				</div>
		...
		<?php
			...
			//si presiono el botón 'ingresar'
			if (isset($_POST['submit']) && !is_numeric($_SESSION['$matriculaa'])) {
				/*
	...
	$o_alumno=new Alumno($_POST['f_mat'],$_POST['f_exp'],$_POST['f_cve'],$_POST['f_gpo']);  // instancio la clase pasandole el nro de cliente para cargar los datos
	...
	$o_alumno->setAulaOrdi($_POST['f_aula']);


	print " Consulta ejecutada: ". $o_alumno->updateMateriaOrdi(); 
				
			}

			// si presiono el botón y es 'eliminar'
			...
			print	'<br/>
								...
								<center>
									...
									while($row = mysql_fetch_array($_pagi_result)) {
										print 	'<tr>'
												...
												.'<td>'.utf8_encode($row['dia_ordi']).' | '.$row['fecha_ordi'].' | '.$row['horai_ordi'].' | '.$row['horaf_ordi'].' | '.$row['aula_ordi'].'</td>'
												.'<td>'.utf8_encode($row['dia_extra']).' | '.$row['fecha_extra'].' | '.$row['horai_extra'].' | '.$row['horaf_extra'].' | '.$row['aula_extra'].'</td>'
												//en este ejemplo para simplificar se envian los parametros por get utilizando un href
												.'<td><a href="log_forma_alumnos_abc.php?exp='.$row['expediente'].'&gpo='.$row['grupo'].'&cve='.$row['clave'].'&md='.$_SESSION['$matriculaa'].'&_pagi_pg='.$_pagi_pg.'"><img src="pics/modificar.png" border="0" style="position:relative;left:12px;"></a></td>'   
												.'</tr>';									}
									echo '</table>';
								echo '</center>';
							echo '</div>';
				...
				//Incluimos la barra de navegación si el "total de resultados" es mayor que el la cantidad de "resultados por página"

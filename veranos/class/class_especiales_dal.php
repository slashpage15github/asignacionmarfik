...
function existe_matricula_materia_carrera($mat,$mate,$car){
      ...
      $car=$this->db_conn->real_escape_string($car);
        ...
        $sql = "select count(*) from veranos where matricula='$mat' and cve_mat='$mate' and carrera='$car'";
    ...
    function existe_matricula_especiales($matr)
    {
        ...
        $this->set_sql(
            "SELECT * FROM veranos WHERE matricula = '$matr'"
        );
    ...
    function insertar($obj){
		$sql = "insert into veranos (matricula,cve_mat,email,telefono,estatus,grado,carrera,nombre) ";
		$sql .= "values(";
    ...
    function num_especiales($matr)
    {
        ...
        $this->set_sql(
            "select count(matricula) as num_especiales from veranos where matricula = '$matr'"
        );
        ...
        if ($this->existe_matricula_especiales($matr) > 0) {
            ...
            $this->set_sql(
                "SELECT matricula, cve_mat, email, telefono, estatus, grado, carrera, nombre 
                        FROM veranos WHERE matricula = '$matr'"
            );

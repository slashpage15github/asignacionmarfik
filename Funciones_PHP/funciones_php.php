...
/*######################################################################################################################################################################################################################################*/
function obtener_matricula_nombre($mat) {
	//$dato="0";
	...
	$query = "select matricula, alumno from alumnos where matricula=$mat limit 1";
	//echo $query;exit;

	$resultado = mysqli_query($conexion, $query);
	$arreglo_alumno = mysqli_fetch_array($resultado);
	$dato = $arreglo_alumno['matricula'].' - '.utf8_encode($arreglo_alumno['alumno']);
...
function saca_menu_desplegable_boostrap($sentencia,$valor,$nombre,$id_valor)
{
$conexion = mysql_connect("localhost", "root", "");
...
$cogidovalor=0;
echo "<select class='form-control' name=$nombre id=$id_valor style='background: white;font-weight:bold;text-align:left;font-size:20px;color:red;'>";
$resultado=mysql_query($sentencia) or die (mysql_error());
while ($fila=mysql_fetch_row($resultado))
{
 	  ...
 	  if ($fila[0]==$valor)
	  	{
		 	echo "<option selected value='".utf8_encode($fila[0])."'>".utf8_encode($fila[1]);
			$cogidovalor=1;
		}
	else
		{
		 	echo "<option value='".utf8_encode($fila[0])."'>".utf8_encode($fila[1])."</option>";
		}
}// fin del while
...
/*######################################################################################################################################################################################################################################*/
function saca_menu_desplegable_boostrap_alumnos($sentencia,$valor,$nombre,$id_valor)
{
$conexion = mysql_connect("localhost", "root", "");
mysql_select_db("asignacionfs_db", $conexion);

$cogidovalor=0;
echo "<select class='form-control' name=$nombre id=$id_valor>";
$resultado=mysql_query($sentencia) or die (mysql_error());
while ($fila=mysql_fetch_row($resultado))
{

...
function get_datos_alumno($matricula){
	$matricula=$this->db_conn->real_escape_string($matricula);

	$this->set_sql("select matricula, alumno, grado, cve_plan from alumnos where materia not like '%INGLES%' and matricula='$matricula' group by matricula");
  ...
  function existeMatalumno($matricula){
      $matricula=$this->db_conn->real_escape_string($matricula);

...
// PARA ACTIVAR EL BUFFER DE LA PAGINA
  ob_start();
...
$sql_expedientes="select expediente,maestro from gruposmaestros
GROUP BY expediente
order by expediente";


$resultado_exp=mysqli_query($conexion,$sql_expedientes) or die (mysqli_error());
$cuantos_exp=mysqli_num_rows($resultado_exp);
//echo 'cuantos expe:'.$cuantos_exp;exit;
$exp=1;
if ($cuantos_exp>0) {
  while ($fila_exp=mysqli_fetch_array($resultado_exp))
  {
    $v_expediente= $fila_exp["expediente"];
    $v_maestro= $fila_exp["maestro"];
...
from gruposmaestros
 where expediente='$v_expediente' order by fecha_ordi,horai_ordi";
//echo $sql;exit;
...
$pdf ->selectFont('clase_pdf/fonts/Helvetica-Bold.afm');
$pdf->addText(704,545,10,'Pag.'.$exp.' de '.$cuantos_exp);
...
$pdf ->selectFont('clase_pdf/fonts/Helvetica.afm');
$pdf->addText(50,530,11,'<b>Expediente:'.$v_expediente.'</b>');
$pdf->addText(50,517,11,'<b>Maestro(a):'.$v_maestro.'</b>');
$pdf ->selectFont('clase_pdf/fonts/Helvetica-Bold.afm');
...
$pdf->ezTable($data,array('Num'=>'Num.',
          ...
          ),'',array('showHeadings'=>1,'shadeCol'=>array(0.9,0.9,0.9),'shaded'=>0,'fontSize'=>9,'showLines'=>2, 'width'=>900,'cols'=>array('Num'=>array('width'=>40,'justification'=>'center'),
...
//echo $v_expediente.'-'.$exp.'<br>';
//echo 'expedientes:'.$exp.'|'.$v_expediente.' - '.'materias:'.$cuantos.'<br>';
$pdf ->selectFont('clase_pdf/fonts/Helvetica.afm');
...
}// end while externo (expedientes)

}//end si hay expedientes num_rows
else{
  echo 'NO HAY EXPEDIENTES QUE PROCESAR, CONSULTE AL ADMINISTRADOR DEL SISTEMA';
  return;
}
